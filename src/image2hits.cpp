...
void image2hits::hog2poselet_hits(const hog_features& hog, poselet_hits_vector& hits, bool verbose) const
{
  boost::timer t;
  ...
  totaltime+=t.elapsed();
  if(verbose)
    cout << "Total num evals:" << count << " detected poselets:" << hits.size() << " in " << totaltime << " sec." << endl;
}
